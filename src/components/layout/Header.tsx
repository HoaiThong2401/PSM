...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl sticky top-0 z-30 border-b border-slate-200/80 dark:border-slate-800/80 px-3 sm:px-6 md:px-8 py-2.5 sm:py-3.5 flex items-center justify-between gap-2 sm:gap-3">
      <div className="min-w-0 flex-1">
        <h1 className="text-sm xs:text-base sm:text-lg md:text-xl font-bold text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-2 truncate">
          <span className="truncate">{title}</span>
        </h1>
        {subtitle && (
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium hidden md:block truncate">
            {subtitle}
          </p>
        )}
      </div>
      ...
      <div className="flex items-center gap-1 sm:gap-2 shrink-0">
        <CycleSelector
          ...
          className="max-w-[125px] xs:max-w-[155px] sm:max-w-[200px] md:max-w-none"
        />
